...
extension String {
  ...
  var bytesWithLength: [UInt8] {
    UInt16(utf8.count).hlBytes + utf8
  }
}
 ...
 */
extension UInt8 {
  private func bitAt(_ offset: UInt8) -> UInt8 {
    (self >> offset) & 0x01
  }
}
...
extension CocoaMQTTFrame {
  ...
  func encodeLength() -> [UInt8] {
    var bytes: [UInt8] = []
    var digit: UInt8 = 0
    var len = UInt32(variableHeader.count + payload.count)
...
struct CocoaMQTTFrameConnect: CocoaMQTTFrame {
  ...
  init(client: CocoaMQTT) {
    self.client = client
    header = CocoaMQTTFrameType.connect.rawValue
  }
...
extension CocoaMQTTFramePublish: CustomStringConvertible {
  var description: String {
    "PUBLISH(msgid: \(msgid ?? 0), topic: \(topic ?? ""), payload: \(payload))"
  }
}
...
struct CocoaMQTTFrameUnsubscribe: CocoaMQTTFrame {
  ...
  init(msgid: UInt16, topic: String) {
    ...
    header = CocoaMQTTFrameType.unsubscribe.rawValue
    self.msgid = msgid
    self.topic = topic
    qos = CocoaMQTTQOS.qos1.rawValue
  }
