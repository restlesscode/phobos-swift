 ...
 */
private enum CocoaMQTTReadTag: Int {
  case header = 0
  case length
  case payload
}
...
extension CocoaMQTT: CocoaMQTTReaderDelegate {
  ...
  func didReceiveUnsubAck(_ reader: CocoaMQTTReader, msgid: UInt16) {
    ...
    for (t, _) in subscriptions where t == topic {
