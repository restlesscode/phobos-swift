...
extension URL {
  ...
  func listExtendedAttributes() throws -> [String] {
    let list: [String] = try withUnsafeFileSystemRepresentation { (fileSystemPath: UnsafePointer<Int8>?) -> [String] in
      ...
      let result: Int = data.withUnsafeMutableBytes { [count = data.count] (body: UnsafeMutableRawBufferPointer) -> Int in
        listxattr(fileSystemPath, UnsafeMutablePointer<Int8>(OpaquePointer(body.baseAddress)), count, 0)
      }
      guard result >= 0 else { throw URL.posixError(errno) }
