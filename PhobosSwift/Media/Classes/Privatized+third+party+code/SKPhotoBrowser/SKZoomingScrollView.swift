...
open class SKZoomingScrollView: UIScrollView {
  ...
  open func setMaxMinZoomScalesForCurrentBounds() {
    ...
    if SKPhotoBrowserOptions.longPhotoWidthMatchScreen && imageView.frame.height >= imageView.frame.width {
      ...
      // I think that we should to get coefficient between device screen width and image width
      // and assign it to maxScale. I made two mode that we will get the same result for different device orientations.
      if UIApplication.pbs_shared?.statusBarOrientation.isPortrait ?? false {
...
extension SKZoomingScrollView {
  private func getViewFramePercent(_ view: UIView, touch: UITouch) -> CGPoint {
  ...
  private func zoomRectForScrollViewWith(_ scale: CGFloat, touchPoint: CGPoint) -> CGRect {
