...
class SKPagingScrollView: UIScrollView {
  ...
  func deleteImage() {
    // index equals 0 because when we slide between photos delete button is hidden and
    // user cannot to touch on delete button. And visible pages number equals 0
    if numberOfPhotos > 0 {
  ...
  func tilePages() {
    ...
    for index: Int in firstIndex...lastIndex {
      if !visiblePages.filter({ $0.tag - pageIndexTagOffset == index }).isEmpty {
        continue
      }
...
extension SKPagingScrollView {
  ...
  private func createCaptionView(_ index: Int) -> SKCaptionView? {
    if let delegate = browser?.delegate, let ownCaptionView = delegate.captionViewForPhotoAtIndex?(index: index) {
