...
open class LineChartRenderer: LineRadarRenderer {
    ...
    if _xBounds.range >= 1 {
      ...
      for j in _xBounds.dropFirst() {
        ...
        cubicPath.addCurve(to: CGPoint(x: CGFloat(cur.x),
                           ...
                           control1: CGPoint(x: cpx,
                                             y: CGFloat(prev.y * phaseY)),
                           control2: CGPoint(x: cpx,
                                             y: CGFloat(cur.y * phaseY)),
                           transform: valueToPixelMatrix)
      }
    }
  ...
  open func drawCubicFill(context: CGContext,
  ...
  open func drawLinearFill(context: CGContext, dataSet: ILineChartDataSet, trans: Transformer, bounds: XBounds)
  {
    ...
    let filled = generateFilledPath(dataSet: dataSet,
        ...
        for j in _xBounds {
          ...
          if dataSet.isDrawValuesEnabled {
            ChartUtils.drawText(context: context,
                                text: formatter.stringForValue(e.y,
                                                               entry: e,
                                                               dataSetIndex: i,
                                                               viewPortHandler: viewPortHandler),
                                point: CGPoint(x: pt.x,
