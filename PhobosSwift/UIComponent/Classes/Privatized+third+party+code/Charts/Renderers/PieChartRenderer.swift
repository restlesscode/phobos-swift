...
open class PieChartRenderer: DataRenderer {
  ...
  @objc open func calculateMinimumRadiusForSpacedSlice(center: CGPoint,
                                                       radius: CGFloat,
                                                       angle: CGFloat,
    ...
    for j in 0..<entryCount {
        ...
        (innerRadius > 0.0 || accountForSliceSpacing)
      {
        if accountForSliceSpacing {
          var minSpacedRadius = calculateMinimumRadiusForSpacedSlice(center: center,
                                                                     radius: radius,
    ...
    for i in 0..<dataSets.count {
      ...
      for j in 0..<dataSet.entryCount {
        ...
        if drawXOutside || drawYOutside {
          ...
          let pt0 = CGPoint(x: line1Radius * sliceXBase + center.x,
                            y: line1Radius * sliceYBase + center.y)

          let pt1 = CGPoint(x: labelRadius * (1 + valueLineLength1) * sliceXBase + center.x,
          ...
          if drawXOutside && drawYOutside {
            ...
            if j < data.entryCount && pe?.label != nil {
              ChartUtils.drawText(context: context,
                                  ...
                                  align: align,
                                  attributes: [NSAttributedString.Key.font: entryLabelFont ?? valueFont,
  ...
  private func drawCenterText(context: CGContext) {
    ...
    if chart.drawCenterTextEnabled && centerAttributedText.length > 0 {
      ...
      let holeRect = CGRect(x: x - innerRadius,
                            y: y - innerRadius,
  ...
  override open func drawHighlighted(context: CGContext, indices: [Highlight]) {
    ...
    for i in 0..<indices.count {
      ...
      if accountForSliceSpacing {
        sliceSpaceRadius = calculateMinimumRadiusForSpacedSlice(center: center,
                                                                radius: radius,
                                                                angle: sliceAngle * CGFloat(phaseY),
  ...
  private func createAccessibleElement(withIndex idx: Int,
    ...
    var elementValueText = formatter.stringForValue(e.y,
                                                    entry: e,
                                                    dataSetIndex: idx,
                                                    viewPortHandler: viewPortHandler)
