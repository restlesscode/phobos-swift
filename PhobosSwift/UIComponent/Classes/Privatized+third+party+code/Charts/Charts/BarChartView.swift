...
open class BarChartView: BarLineChartViewBase, BarChartDataProvider {
  ...
  override internal func calcMinMax() {
    ...
    leftAxis.calculate(min: data.getYMin(axis: .left),
                       max: data.getYMax(axis: .left))
    rightAxis.calculate(min: data.getYMin(axis: .right),
  ...
  open var isDrawBarShadowEnabled: Bool { drawBarShadowEnabled }
}
