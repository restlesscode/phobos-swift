...
open class CombinedChartView: BarLineChartViewBase, CombinedChartDataProvider {
  ...
  @objc open var drawValueAboveBarEnabled: Bool {
    get { (renderer as! CombinedChartRenderer).drawValueAboveBarEnabled }
  ...
  @objc open var drawBarShadowEnabled: Bool {
    get { (renderer as! CombinedChartRenderer).drawBarShadowEnabled }
    set { (renderer as! CombinedChartRenderer).drawBarShadowEnabled = newValue }
  }

  /// `true` if drawing values above bars is enabled, `false` ifnot
  open var isDrawValueAboveBarEnabled: Bool { (renderer as! CombinedChartRenderer).drawValueAboveBarEnabled }

  /// `true` if drawing shadows (maxvalue) for each bar is enabled, `false` ifnot
  open var isDrawBarShadowEnabled: Bool { (renderer as! CombinedChartRenderer).drawBarShadowEnabled }
  ...
  @objc open var drawOrder: [Int] {
    get {
      (renderer as! CombinedChartRenderer).drawOrder.map { $0.rawValue }
    }
    set {
