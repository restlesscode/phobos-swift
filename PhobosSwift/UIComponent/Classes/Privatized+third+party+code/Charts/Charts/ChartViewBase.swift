...
open class ChartViewBase: NSUIView, ChartDataProvider, AnimatorDelegate {
  ...
  /// (use the `marker` property to specify a marker)
  @objc open var isDrawMarkersEnabled: Bool { drawMarkers }
  ...
  internal func drawDescription(in context: CGContext) {
    ...
    let attrs: [NSAttributedString.Key: Any] = [.font: description.font,
  ...
  override open func accessibilityChildren() -> [Any]? {
    renderer?.accessibleChartElements
  }
  ...
  @objc open var midPoint: CGPoint {
    CGPoint(x: bounds.origin.x + bounds.size.width / 2.0, y: bounds.origin.y + bounds.size.height / 2.0)
  }
