...
class FloatingPanelCore: NSObject, UIGestureRecognizerDelegate {
  ...
  @objc func handle(panGesture: UIPanGestureRecognizer) {
    ...
    case scrollView?.panGestureRecognizer:
      ...
      let belowEdgeMost = layoutAdapter.offsetFromEdgeMost + (1.0 / surfaceView.fp_displayScale) < 0
      ...
      if belowEdgeMost {
        ...
        if interactionInProgress {
          ...
          if state == layoutAdapter.edgeMostState, animator == nil {
  ...
  private func interruptAnimationIfNeeded() {
    if let animator = moveAnimator, animator.isRunning {
  ...
  private func shouldScrollViewHandleTouch(_ scrollView: UIScrollView?, point: CGPoint, velocity: CGFloat) -> Bool {
    ...
    guard
      ...
      layoutAdapter.offsetFromEdgeMost == 0
    else {
      return false
    }
  ...
  private func panningEnd(with translation: CGPoint, velocity: CGPoint) {
    ...
    stopScrollDeceleration = (layoutAdapter.offsetFromEdgeMost + (1.0 / surfaceView.fp_displayScale) < 0) // Projecting the dragging to the scroll dragging or not
    ...
    guard shouldAttract(to: targetPosition) else {
      ...
      updateLayout(to: targetPosition)
      unlockScrollView()
      return
    }
  ...
  private func move(to targetPosition: FloatingPanelState, with velocity: CGFloat, completion: @escaping (() -> Void)) {
    ...
    moveAnimator = NumericSpringAnimator(initialData: initialData,
                                         target: target,
  ...
  func value(of point: CGPoint) -> CGFloat {
    layoutAdapter.position.mainLocation(point)
  }
  ...
  override func responds(to aSelector: Selector!) -> Bool {
    super.responds(to: aSelector) || panGestureRecognizer.delegateProxy?.responds(to: aSelector) == true
  }
...
public final class FloatingPanelPanGestureRecognizer: UIPanGestureRecognizer {
  ...
  public weak var delegateProxy: UIGestureRecognizerDelegate? {
    didSet {
      delegate = floatingPanel // Update the cached IMP
    }
  }
}
...
private class NumericSpringAnimator: NSObject {
  ...
  init(initialData: Data,
    ...
    zeta = abs(initialData.velocity) > 300 ? CoreGraphics.log(decelerationRate) / (-2.0 * .pi * frequency * duration) : 1.0
