...
@objc public protocol FloatingPanelControllerDelegate: AnyObject {
...
open class FloatingPanelController: UIViewController {
  ...
  public var panGestureRecognizer: FloatingPanelPanGestureRecognizer {
    floatingPanel.panGestureRecognizer
  }
  ...
  public var adjustedContentInsets: UIEdgeInsets {
    floatingPanel.layoutAdapter.adjustedContentInsets
  }
  ...
  public var isRemovalInteractionEnabled: Bool {
    ...
    @objc(isRemovalInteractionEnabled) get { floatingPanel.isRemovalInteractionEnabled }
  }
  ...
  override open func loadView() {
    assert(storyboard == nil, "Storyboard isn't supported")
  ...
  override open func viewDidLayoutSubviews() {
    ...
    if #available(iOS 11.0, *) {
      ...
      if floatingPanel.isAttracting == false {
        update(safeAreaInsets: fp_safeAreaInsets)
      }
    }
  }
  ...
  override open func viewWillTransition(to size: CGSize, with coordinator: UIViewControllerTransitionCoordinator) {
    ...
    if let newLayout = delegate?.floatingPanel?(self, layoutFor: size) {
      layout = newLayout
      activateLayout(forceLayout: false)
    }
  ...
  override open func willTransition(to newCollection: UITraitCollection, with coordinator: UIViewControllerTransitionCoordinator) {
    ...
    if let newLayout = delegate?.floatingPanel?(self, layoutFor: newCollection) {
      layout = newLayout
      activateLayout(forceLayout: false)
    }
  }
  ...
  private func shouldUpdateLayout(from previous: UITraitCollection, to new: UITraitCollection) -> Bool {
    previous.horizontalSizeClass != new.horizontalSizeClass
  ...
  public func addPanel(toParent parent: UIViewController, at viewIndex: Int = -1, animated: Bool = false) {
    ...
    NSLayoutConstraint.activate([view.topAnchor.constraint(equalTo: parent.view.topAnchor, constant: 0.0),
                                 view.leftAnchor.constraint(equalTo: parent.view.leftAnchor, constant: 0.0),
                                 view.rightAnchor.constraint(equalTo: parent.view.rightAnchor, constant: 0.0),
                                 view.bottomAnchor.constraint(equalTo: parent.view.bottomAnchor, constant: 0.0)])
  ...
  public func surfaceLocation(for state: FloatingPanelState) -> CGPoint {
    floatingPanel.layoutAdapter.surfaceLocation(for: state)
  }
  ...
  @objc public func fp_dismiss(animated flag: Bool, completion: (() -> Void)? = nil) {
    ...
    if let fpc = parent as? FloatingPanelController {
      if fpc.presentingViewController != nil {
        fp_original_dismiss(animated: flag, completion: completion)
      } else {
