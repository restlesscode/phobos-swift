...
class LayoutAdapter {
  ...
  private var surfaceView: FloatingPanelSurfaceView {
    vc.surfaceView
  }

  private var backdropView: FloatingPanelBackdropView {
    vc.backdropView
  }

  private var safeAreaInsets: UIEdgeInsets {
    vc?.fp_safeAreaInsets ?? .zero
  }
  ...
  var orderedStates: [FloatingPanelState] {
    activeStates.sorted(by: {
      $0.order < $1.order
    })
  }

  var validStates: Set<FloatingPanelState> {
    activeStates.union([.hidden])
  }
  ...
  private var directionalLeastState: FloatingPanelState {
    sortedDirectionalStates.first ?? .hidden
  }

  private var directionalMostState: FloatingPanelState {
    sortedDirectionalStates.last ?? .hidden
  }
  ...
  func position(for state: FloatingPanelState) -> CGFloat {
    ...
    let anchor = layout.anchors[state] ?? hiddenAnchor
  ...
  func isIntrinsicAnchor(state: FloatingPanelState) -> Bool {
    layout.anchors[state] is FloatingPanelIntrinsicLayoutAnchor
  }
  ...
  func prepareLayout() {
    ...
    if let constraints = layout.prepareLayout?(surfaceView: surfaceView, in: vc.view) {
      ...
      switch position {
      case .top, .bottom:
        surfaceConstraints = [surfaceView.leftAnchor.constraint(equalTo: vc.fp_safeAreaLayoutGuide.leftAnchor, constant: 0.0),
                              surfaceView.rightAnchor.constraint(equalTo: vc.fp_safeAreaLayoutGuide.rightAnchor, constant: 0.0)]
      case .left, .right:
        surfaceConstraints = [surfaceView.topAnchor.constraint(equalTo: vc.fp_safeAreaLayoutGuide.topAnchor, constant: 0.0),
                              surfaceView.bottomAnchor.constraint(equalTo: vc.fp_safeAreaLayoutGuide.bottomAnchor, constant: 0.0)]
      }
    }
    let backdropConstraints = [backdropView.topAnchor.constraint(equalTo: vc.view.topAnchor, constant: 0.0),
    ...
    NSLayoutConstraint.deactivate(constraint: fitToBoundsConstraint)
    fitToBoundsConstraint = nil
  ...
  func setUpAttraction(to state: FloatingPanelState) -> (NSLayoutConstraint, CGFloat) {
    ...
    attractionConstraint = animationConstraint
  ...
  func updateStaticConstraint() {
    ...
    let anchor = layout.anchors[edgeMostState]!
    ...
    default:
      switch position {
      case .top, .left:
        staticConstraint = surfaceAnchor.constraint(equalToConstant: position(for: directionalMostState))
      case .bottom, .right:
        let rootViewAnchor = position.mainDimensionAnchor(vc.view)
        staticConstraint = rootViewAnchor.constraint(equalTo: surfaceAnchor,
                                                     constant: position(for: directionalLeastState))
      }
    }
  ...
  private func rubberBandEffect(for buffer: CGFloat, base: CGFloat) -> CGFloat {
    (1.0 - (1.0 / ((buffer * 0.55 / base) + 1.0))) * base
  }
  ...
  func activateLayout(for state: FloatingPanelState, forceLayout: Bool = false) {
    ...
    if vc.contentMode == .fitToBounds {
      NSLayoutConstraint.activate(constraint: fitToBoundsConstraint)
    }
  ...
  private func setBackdropAlpha(of target: FloatingPanelState) {
    if target == .hidden {
      ...
      backdropView.alpha = backdropAlpha(for: target)
    }
  }

  func backdropAlpha(for state: FloatingPanelState) -> CGFloat {
    layout.backdropAlpha?(for: state) ?? defaultLayout.backdropAlpha(for: state)
  }

  fileprivate func checkLayout() {
    // Verify layout configurations
    assert(activeStates.isEmpty == false)
