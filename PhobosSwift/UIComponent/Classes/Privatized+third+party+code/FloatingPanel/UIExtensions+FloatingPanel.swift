...
internal func displayEqual(_ lhs: CGFloat, _ rhs: CGFloat, by displayScale: CGFloat) -> Bool {
  displayTrunc(lhs, by: displayScale) == displayTrunc(rhs, by: displayScale)
}
...
extension UIScrollView {
  var isLocked: Bool {
    !showsVerticalScrollIndicator && !bounces && isDirectionalLockEnabled
  }
  ...
  var fp_contentOffsetMax: CGPoint {
    CGPoint(x: max((contentSize.width + fp_contentInset.right) - bounds.width, 0.0),
...
extension CGPoint {
  static var leastNonzeroMagnitude: CGPoint {
    CGPoint(x: CGFloat.leastNonzeroMagnitude, y: CGFloat.leastNonzeroMagnitude)
  }
